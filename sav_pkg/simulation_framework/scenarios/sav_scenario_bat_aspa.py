...
class SAVScenarioBATASPA(SAVScenario):
    ...
    def _get_randomized_non_default_asn_cls_dict(
        ...
        # All BAR-SAV ASes adopt ASPA 
        for asn, sav_policy in self.sav_policy_asn_dict.items():
            if sav_policy == self.scenario_config.BaseSAVPolicyCls:
                asn_cls_dict[asn] = ASPAFull

        # Random adoption of TE by transit ASes (ETC = not edge or tier 1)

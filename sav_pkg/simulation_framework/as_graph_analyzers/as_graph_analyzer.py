...
class SAVASGraphAnalyzer(BaseASGraphAnalyzer):
    ...
    def _get_victim_outcome_data_plane(self, as_obj):
        ...
        for reflector_asn in self.scenario.reflector_asns:
            self._propagate_packet(as_obj, source_prefix, prev_hop, origin, reflector_asn)
    ...
    def _propagate_packet(self, as_obj, source_prefix, prev_hop, origin, dst, filtered=False):
        """
        ...
        if outcome_int is None:
            outcome_int = self._determine_as_outcome_data_plane(as_obj, source_prefix, prev_hop, origin, filtered)
            self._data_plane_outcomes[(as_obj.asn, source_prefix, prev_hop_asn, origin)] = outcome_int
        else:
            new_outcome_int = self._determine_as_outcome_data_plane(as_obj, source_prefix, prev_hop, origin, filtered)
            if outcome_int != new_outcome_int:
                self._data_plane_outcomes[(as_obj.asn, source_prefix, prev_hop_asn, origin)] = min(outcome_int, new_outcome_int)
        ...
        for ann in as_obj.policy._local_rib.data.values():
            ...
            if ann.origin == dst:
                ...
                if as_obj.asn != dst and dst_ann is not None:
                    ...
                    self._propagate_packet(as_obj, source_prefix, prev_hop, origin, dst, filtered)


    def _determine_as_outcome_data_plane(self, as_obj, source_prefix, prev_hop, origin, filtered=False):
        ...
        if as_obj.asn in self.scenario.sav_policy_asn_dict:
            ...
            validated = sav_policy.validate(as_obj, source_prefix, prev_hop, self.engine, self.scenario)
            if validated:
                return Outcomes.FALSE_NEGATIVE.value if spoofed_packet else Outcomes.TRUE_NEGATIVE.value
            else:
                return Outcomes.TRUE_POSITIVE.value if spoofed_packet else Outcomes.FALSE_POSITIVE.value
        ...
        for reflector_asn in self.scenario.reflector_asns:
            for attacker_asn in self.scenario.attacker_asns:
                if not self._has_outcome(reflector_asn, attacker_asn):
                    self._data_plane_outcomes[(reflector_asn, None, None, attacker_asn)] = Outcomes.DISCONNECTED.value
            for victim_asn in self.scenario.victim_asns:
                if not self._has_outcome(reflector_asn, victim_asn):
                    self._data_plane_outcomes[(reflector_asn, None, None, victim_asn)] = Outcomes.DISCONNECTED.value
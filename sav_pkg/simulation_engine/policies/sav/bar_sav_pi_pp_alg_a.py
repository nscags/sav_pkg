...
class BAR_SAV_PI_PP_Alg_A(BaseSAVPolicy):
    name: str = "BAR-SAV-PI++ Algorithm A"
    ...
    def _validate(
        ...
        source_asns = BAR_SAV_PI_PP_Alg_A._get_source_asns(as_obj, source_prefix)
        if not source_asns:
        ...
        inferred_provider_relationships, inferred_peer_relationships, ambiguous_relationships = BAR_SAV_PI_PP_Alg_A._infer_relationships_from_paths(
        ...
        D_f, P_f = BAR_SAV_PI_PP_Alg_A._get_f_provider_cone(
        ...
        for source_asn in source_asns:
            p_of_s, trace = BAR_SAV_PI_PP_Alg_A._compute_p_of_source(
                as_obj,
                source_asn,
                engine,
                D_f,
        ...
        for provider_asn in as_obj.provider_asns:
            for ann_info in as_obj.policy.ribs_in.data.get(provider_asn, {}).values():
                if as_obj.policy._valid_ann(
                    ...
                    peak_asns = BAR_SAV_PI_PP_Alg_A._determine_peak(as_obj, engine, tier1_asns, as_path)
                    BAR_SAV_PI_PP_Alg_A._infer_relationships_from_peak(
                        as_obj, 
                        as_dict, 
                        as_path, 
                        peak_asns,
    ...
    def _compute_p_of_source(
        ...
        for y_asn in sorted(max_dist_from_s, key=lambda x: max_dist_from_s[x], reverse=True):
            ...
            if not confirmed_p_f:
                known_peers, peer_src = BAR_SAV_PI_PP_Alg_A._get_known_peers(
                ...
                for peer_asn in sorted(known_peers):
                    if peer_asn in D_f:
                        ...
                        if best_dist is None or candidate_dist < best_dist:
                            ...
                            case_used = f"B({peer_src},via={peer_asn})"
                        elif candidate_dist == best_dist:
                            confirmed_p_f.update(P_f[peer_asn])
                            case_used = f"B({peer_src},merged,via={peer_asn})"
            ...
            y_asn_ambiguous_p_f = BAR_SAV_PI_PP_Alg_A._get_ambiguous_p_f(
                as_obj,
                y_asn,
                ambiguous_relationships,
            )
            ...
            gt = BAR_SAV_PI_PP_Alg_A._gt_ingress_at_f(as_obj, y_asn, engine, scenario)
            if gt is not None:
    ...
    def _get_known_peers(
        ...
        """ 
        """
        inferred = inferred_peer_relationships.get(y_asn, frozenset())

        if y_as is None:
            return frozenset(inferred), "inferred"

        # y publishes ASRA, its own record is complete
        if isinstance(y_as.policy, ASRA):
            return frozenset(y_as.peer_asns), "ASRA"

        known: set[int] = set(inferred)
        attested_by_peer = False
        for peer_asn in y_as.peer_asns:
            peer_as = as_dict.get(peer_asn)
            if (
                peer_as is not None
                and isinstance(peer_as.policy, ASRA)
                and y_asn in peer_as.peer_asns
            ):
                known.add(peer_asn)
                attested_by_peer = True

        if attested_by_peer:
            return frozenset(known), "peerASRA"
        return frozenset(known), "inferred"

from .framework import framework_test_configs

from .bgp import bgp_test_configs

from .strict import strict_test_configs
from .fp import fp_test_configs
from .efp_a import efp_a_test_configs
from .efp_a_w_peers import efp_a_w_peers_test_configs
from .efp_b import efp_b_test_configs
from .bar_sav import bar_sav_test_configs
from .bspi import bspi_test_configs
from .bar_sav_pp import bar_sav_pp_test_configs
from .bspi_pp import bspi_pp_test_configs

from .dsr import dsr_test_configs


engine_test_configs = (
    # + framework_test_configs 
    # + bgp_test_configs
    # + strict_test_configs 
    # + fp_test_configs 
    # + efp_a_test_configs 
    # + efp_a_w_peers_test_configs 
    # + efp_b_test_configs 
    # + bar_sav_test_configs 
    # + bspi_test_configs
    # + bar_sav_pp_test_configs 
    bspi_pp_test_configs
    # + dsr_test_configs
)

__all__ = ["engine_test_configs"]